...
Two sources in the 2026-06-08 batch treat AI infrastructure spending as both the engine of the current cycle and a source of sharp market instability. The buildout (datacenters, GPUs, power, frontier hiring, mega-IPOs) is so large that financing it reshapes capital flows, and any doubt about returns can move many linked assets at once. The constraint is financial gravity: spend is massive but not unlimited, and ROI may be hard to even measure.
...
- Optimism vs. gravity: the broadly pro-acceleration framing (capex as engine of progress) sits against the warning that returns may not justify the spend. The sources hold both without resolving which dominates — appropriate given these are forecasts.
