# Layoffs as Strategy Signals

## Summary

"AI layoffs" are not a single phenomenon. Two sources in the 2026-06-08 batch argue the same headline can mean very different things, and that the useful move is to decode which constraint a company is revealing rather than treating any cut as proof that AI replaced workers.
...
- No contradiction between the two sources — they reinforce each other. The tension is external: the framework asks for operational proof (metrics, workflow changes) that public layoff announcements rarely disclose, so classification is often a judgment call under missing data.

## Open Questions

- How do you classify a layoff that mixes categories (capex pressure + performance enforcement + ordinary weakness)?
- What public signals reliably distinguish a "visionary redesign" from a "hope layoff" before the metrics are in?
